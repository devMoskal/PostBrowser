...
internal class GetPostTest : BaseTest() {
    ...
    fun `when repository return success then propagate propagate result`() = runBlockingTest {
        // given
        val data = mockk<Post>()
        coEvery { mockRepository.getPost(any()) } returns flowOf(Resource.Success(data))
        // when
        val result = getPost.execute(42).first()
        // then
    ...
    fun `when repository return error then propagate propagate result`() = runBlockingTest {
        ...
        coEvery { mockRepository.getPost(any()) } returns flowOf(Resource.Error(error))
        // when
        val result = getPost.execute(42).first()
        // then
